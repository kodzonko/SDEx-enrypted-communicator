...
export default function Chat() {
    ...
    React.useEffect(() => {
        if (socket.connected && contact?.publicKey) {
            logger.info(`Checking if user ${contact.getFullName()} is online.`);
            (async () => {
                if (await checkOnline(contact.publicKey)) {
    ...
    }, [socket.connected, contact?.publicKey]);

    // If user is online, send chatInit message
    React.useEffect(() => {
        if (contact?.publicKey) {
            (async () => {
                const isOnline = await checkOnline(contact.publicKey);
                if (isOnline) {
    ...
    function onSend(newMessages: GiftedChatMessage[] = []) {
        ...
        (async () => {
            const thirdPartyOnline = contact?.publicKey
                ? await checkOnline(contact?.publicKey)
                : false;
            if (
                !contact ||
                !socket.connected ||
    ...
    const determineBannerText = (): string => {
        ...
        (async () => {
            ...
            const isOnline = contact ? await checkOnline(contact?.publicKey) : false;
            if (!isOnline) {
                text =
